    ...
    try
    {
      if (ser.isOpen())
      {
        ...
        if(ser.available())
        {
          ...
          input += read;
          while (input.length() >= 55) // while there might be a complete package in input
          {
            //parse for data packets
            data_packet_start = input.find("$\x3A");
            if (data_packet_start != std::string::npos)
            {
              ROS_DEBUG("found possible start of data packet at position %d", data_packet_start);
              if ((input.length() >= data_packet_start + 55) && (input.compare(data_packet_start + 53, 2, "\r\n") == 0))  //check if positions 53,54 exist, then test values
              {
                ROS_DEBUG("seems to be a real data package: long enough and found end characters");
                // get quaternion values
                int16_t w = (((0xff &(char)input[data_packet_start + 30]) << 8) | 0xff &(char)input[data_packet_start + 29]);
                int16_t x = (((0xff &(char)input[data_packet_start + 32]) << 8) | 0xff &(char)input[data_packet_start + 31]);
                int16_t y = (((0xff &(char)input[data_packet_start + 34]) << 8) | 0xff &(char)input[data_packet_start + 33]);
                int16_t z = (((0xff &(char)input[data_packet_start + 36]) << 8) | 0xff &(char)input[data_packet_start + 35]);
                ...
                input.erase(0, data_packet_start + 55); // delete everything up to and including the processed packet
              }
              else
              {
                if (input.length() >= data_packet_start + 55)
                {
